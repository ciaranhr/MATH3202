...
def travelling_artist(i, S, n):
    if n == 0 or len(S) == 4:
        return (-cost[i][Home], Home,  names[Home])
    else:
        return max((-cost[i][j] +\
              sum(p[j][k]*(Price*min(k,n) + travelling_artist(j,S+[j], n-min(k,n))[0]) for k in K), j, names[j])
                for j in Cities if j not in S)

print(travelling_artist(0, [], 5))
