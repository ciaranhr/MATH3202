...
#A Neighbouring solution will be a board with two pieces swapped
def ChoosePieces(B):
    k1 = tuple(random.choices(I, k=2))
    k2 = tuple(random.choices(I, k=2))
    cost = BoardCost(B)
    B[k1], B[k2] = B[k2], B[k1]
    delta = BoardCost(B) - cost
    B[k1], B[k2] = B[k2], B[k1]
    return (delta, (k1, k2))
...
RunSA(Board, BoardCost, ChoosePieces, SwapPieces, 20, 100000, .9999)
